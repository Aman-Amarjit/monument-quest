...
export class FeedController {
  static async getFeed(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const result = await FeedService.getFeed(req.user?.id, req.query.limit, typeof req.query.cursor === 'string' ? req.query.cursor : undefined, typeof req.query.scope === 'string' ? req.query.scope : undefined);
  ...
  static async deletePost(req: AuthRequest, res: Response, next: NextFunction) {
    try { res.json(await FeedService.deletePost(req.params.id, req.user!.id)); } catch (error) { next(error); }
  }

  static async toggleLike(req: AuthRequest, res: Response, next: NextFunction) {
    try { res.json({ success: true, data: await FeedService.toggleLike(req.params.id, req.user!.id) }); } catch (error) { next(error); }
  }

  static async getComments(req: AuthRequest, res: Response, next: NextFunction) {
    try { const data = await FeedService.getComments(req.params.id, req.query.limit); res.json({ success: true, count: data.length, data }); } catch (error) { next(error); }
  }
